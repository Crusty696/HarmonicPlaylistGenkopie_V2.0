...
def main() -> int:
    ...
    candidates = []
    audio_dir = os.environ.get("HPG_TEST_AUDIO_DIR", r"D:\beatport_tracks_2025-08")
    for pattern in ("tests/**/*.aiff", "tests/**/*.wav", "tests/**/*.mp3",
                    "validation/**/*.aiff", "validation/**/*.wav",
                    os.path.join(audio_dir, "*.aiff"),
                    os.path.join(audio_dir, "*.wav"),
                    os.path.join(audio_dir, "*.mp3")):
        candidates.extend(glob.glob(pattern, recursive=True))
    candidates = [c for c in candidates if os.path.getsize(c) > 500_000]
    ...
    if len(tracks) < 2:
        print("E2E FEHLER: mindestens 2 Tracks muessen analysierbar sein")
        return 1
    else:
        # --- 2) Playlist + Empfehlungen ---
        ordered = generate_playlist(tracks, "Harmonic Flow", bpm_tolerance=16.0)
        check("Playlist generiert", len(ordered) == len(tracks), f"{len(ordered)} Tracks")
        ctx = resolve_scoring_context("Harmonic Flow", {})
        recs = compute_transition_recommendations(
            ordered, bpm_tolerance=16.0, scoring_context=ctx
        )
