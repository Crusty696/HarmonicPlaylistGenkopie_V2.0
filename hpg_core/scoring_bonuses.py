...
class EnhancedBonusCalculator:
  ...
  def calculate_all_bonuses(
    ...
    bonus += EnhancedBonusCalculator._danceability_bonus(current, candidate, context)

    # === PEAK MOMENTUM BONUS ===
    if context.get_playlist_phase() == "PEAK":
      bonus += EnhancedBonusCalculator._peak_momentum_bonus(current, candidate, context)

    # === STRATEGY-SPECIFIC BONUSES ===
    if strategy == "EMOTIONAL_JOURNEY":
      bonus += EnhancedBonusCalculator._emotional_journey_bonus(current, candidate)

    elif strategy == "GENRE_FLOW":
      bonus += EnhancedBonusCalculator._genre_storytelling_bonus(current, candidate)

    return min(0.2, bonus)

  @staticmethod
  def _surprise_bonus(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
    ...
    if energy_trend == "RISING" and candidate.energy > current.energy and camelot_dist <= 2:
      bonus += 0.04

    return bonus

  @staticmethod
  def _flow_bonus(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
  ...
  @staticmethod
  def _danceability_bonus(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
  ...
  @staticmethod
  def _peak_momentum_bonus(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
  ...
  def _genre_storytelling_bonus(current: Track, candidate: Track) -> float:
    ...
    compat = GenreCompatibilityMatrix.get_compatibility(current_genre, candidate_genre)
...
class EnhancedPenaltyCalculator:
  ...
  def calculate_all_penalties(
    ...
    # === JARRING TRANSITIONS ===
    penalty += EnhancedPenaltyCalculator._jarring_penalty(current, candidate, context)

    # === REPETITION PENALTY (erweitert) ===
    penalty += EnhancedPenaltyCalculator._repetition_penalty(current, candidate, context)

    # === ENERGY CLIFF PENALTY ===
    penalty += EnhancedPenaltyCalculator._energy_cliff_penalty(current, candidate, context)

    # === SPECTRAL MONOTONY PENALTY ===
    penalty += EnhancedPenaltyCalculator._spectral_monotony_penalty(current, candidate, context)

    # === TONAL DRIFT PENALTY ===
    penalty += EnhancedPenaltyCalculator._tonal_drift_penalty(current, candidate, context)

    return max(-0.3, penalty)

  @staticmethod
  def _jarring_penalty(current: Track, candidate: Track, context: PlaylistContext) -> float:
  ...
  @staticmethod
  def _repetition_penalty(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
  ...
  @staticmethod
  def _energy_cliff_penalty(current: Track, candidate: Track, context: PlaylistContext) -> float:
  ...
  @staticmethod
  def _spectral_monotony_penalty(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
    ...
    if recent_diff < 10 and abs(candidate_brightness - recent_brightnesses[-1]) < 10:
      penalty -= 0.05  # "Zu viel gleiche Farbe"

    return penalty

  @staticmethod
  def _tonal_drift_penalty(current: Track, candidate: Track, context: PlaylistContext) -> float:
    """
