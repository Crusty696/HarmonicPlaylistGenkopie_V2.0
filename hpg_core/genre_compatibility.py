...
class GenreCompatibilityMatrix:
    ...
    def get_compatibility(genre1: str, genre2: str) -> float:
        ...
        for (
            stored_g1,
            stored_g2,
        ), score in GenreCompatibilityMatrix.COMPATIBILITY.items():
        ...
        if GenreCompatibilityMatrix._is_electronic(
            g1
        ) and GenreCompatibilityMatrix._is_electronic(g2):
            return 0.65  # Electronic-Genres sind meist kompatibel

        if GenreCompatibilityMatrix._is_acoustic(
            g1
        ) and GenreCompatibilityMatrix._is_acoustic(g2):
