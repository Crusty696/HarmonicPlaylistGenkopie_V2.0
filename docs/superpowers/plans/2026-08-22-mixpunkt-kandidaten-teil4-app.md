...
1. **"Track.mix_in_point/mix_out_point = Rang 1"** wird **nicht** durch Mutation der Track-Objekte umgesetzt (Rang 1 haengt vom Partner ab; Tracks liegen im Cache und in mehreren Playlist-Varianten). Stattdessen tragen `TransitionPlan.mix_out_a/mix_in_b/overlap` den aktiven Kandidaten, und **alle** Leser lesen den Plan: Preview (`resolve_transition_mix_points` bevorzugt plan — heute schon), Timeline (`rec.plan` — heute schon), Tabelle Mix-In/Out (neu: aus den Empfehlungen des Paars, sonst Track-Wert), Export (neu: Empfehlungen werden uebergeben). Track-Felder bleiben die Analyse-Werte.
...
7. `scoring_context` (HPG-001): die Kandidaten-Wahl liegt **nicht** im Kontext-Dict (sie wird je Paar aus der Datei gelesen, identisch in allen fuenf Konsumenten); der Kontext bleibt `harmonic_strictness/allow_experimental/target_energy/overlap`. `_enhanced_cache_key` bleibt; nach einem Klick im Panel wird nicht neu sortiert, sondern nur `compute_transition_recommendations` + Quality + Panels neu berechnet (Caches ausserhalb `generate_playlist` sind None — kein Stale-Risiko). Die Sortierung beim **naechsten** Lauf sieht die Wahl (select_pair_candidate in `calculate_enhanced_compatibility`) — Spec: "beim naechsten Lauf bevorzugt".
8. GUI (nur das Noetige): in jeder Uebergangs-Karte eine `QTableWidget` "Kandidaten" (Spalten: Rang, Mix-Out A [s], Mix-In B [s], Blende [Takte], Schema (out → in), Score, Teilwerte (Kurzform `H .75 T 1.0 E .98 G 1.0 Gr .83 B .60 K .72 S .99 L .77 St .07`), Begruendung als Tooltip der Zeile); aktive Zeile markiert; Klick/Enter → Signal `candidate_chosen(index, rang)`. MainWindow: Wahl merken (`candidate_choices.merke`), Empfehlungen neu berechnen (`compute_transition_recommendations` mit `transition_metrics` neu, weil der Score des Paars von der Wahl abhaengt → `compute_adjacent_transition_metrics` ebenfalls neu), Quality, Panels verteilen — derselbe Pfad wie nach der Generierung (neue Methode `_verteile_uebergaenge(bpm_tolerance, scoring_context)`, die `analysis_finished` und `_on_playlist_reordered` benutzen). Preview-Cache des betroffenen Paars wird verworfen (Clip gehoert zum alten Plan).
9. Tabelle Mix-In/Mix-Out (`PlaylistPanel._populate_table`): zeigt fuer Track i den Mix-In aus der Empfehlung (i−1 → i) und den Mix-Out aus (i → i+1), wenn vorhanden (`rec.plan.mix_in_b`/`mix_out_a`), sonst Track-Wert; Tooltip nennt die Quelle ("Kandidat Rang n" / "Analyse").
10. Regler: neuer Slider "Lautheit (Kandidaten)" mit Schluessel `kandidaten_loudness_weight`; `tolerances.write_override_kandidaten(gewichte)` schreibt die uebergebenen `kandidaten_*_weight` und skaliert die uebrigen neun proportional auf Rest (Summe 1.0, Validierung `_gueltig`); `_on_transition_weight_changed` trennt Schluessel nach Praefix. `candidate_preferences` (Hoertest) hat Vorrang vor Toleranzen in `score_pair` (Teil 3) — der Regler wirkt also nur, solange keine Praeferenz fuer das Genre vorliegt; die Statuszeile sagt das ("Hoertest-Praeferenz aktiv fuer: Psytrance — Regler ohne Wirkung dort").
11. Export: `RekordboxXMLExporter.export(playlist, output_path, playlist_name, transitions=None)`; mit `transitions` (Liste `TransitionRecommendation`, Laenge n−1): MIX OUT von Track i = `transitions[i].plan.mix_out_a`, MIX IN von Track i = `transitions[i-1].plan.mix_in_b` (sonst Track-Wert), zusaetzlich Memory-Cues `HPG K<rang> OUT <schema_out>` auf Track A und `HPG K<rang> IN <schema_in>` auf Track B fuer alle Kandidaten der Empfehlung (max. 6 Kombinationen → Rang der Kombination, gleiche Zeitpunkte einmal), **nur bei `outro_covered`** (Gate `_cue_export_allowed` bleibt). m3u8 unveraendert (kennt keine Cues). `main._export_rekordbox_xml` uebergibt `self.playlist_panel.transition_recommendations`.
12. App-BPM-Default 2.0 (Spec): `main.py` Slider `setValue(2)`, Label "±2", Tooltip "±2 BPM (Hoertest-Gate)"; `current_bpm_tolerance = 2.0`; `AnalysisWorker` Default 2.0; `PlaylistPanel` Defaults 2.0. `playlist.py`-API-Defaults (3.0) bleiben (Bibliotheks-API, nicht App). Bestandstests, die den App-Default 3 pruefen, werden auf 2 umgestellt und im Handoff als Spec-Aenderung benannt.
13. Laufzeit: `select_pair_candidate` wird je Paar in `calculate_enhanced_compatibility` gerufen — in `generate_playlist` dank `_ENHANCED_COMPAT_CACHE` hoechstens einmal je (Paar, Kontext); zusaetzlich Modul-Cache `_PAIR_CANDIDATE_CACHE` (nur waehrend `generate_playlist` aktiv, wie die anderen) fuer `rank_pair_candidates`, weil Panel/Export dieselbe Liste brauchen. Messung (Task 10) an den 231 Tracks: Generierungszeit vorher/nachher, keine stille Kappung; wird es zu langsam, steht die Zahl im Handoff und die Entscheidung beim Nutzer.
...
- [ ] **Step 3: Implementierung in `main.py`** (4 Leerzeichen):
  ...
  - `MainWindow._verteile_uebergaenge(bpm_tolerance, scoring_context)`: berechnet `compute_adjacent_transition_metrics`, `calculate_playlist_quality`, `compute_transition_recommendations`, setzt `playlist_panel` (Quality, Recommendations, `set_playlist_data`), `mix_tips_panel.set_recommendations` + `setup_transition_previews`, `timeline_panel.set_timeline`, `analytics_panel.set_analytics`, Toolbar-Quality. `analysis_finished` (:4812-4860) und `_on_playlist_reordered` (:5008-5030) benutzen sie (Verhalten gleich, nur zusammengefasst).
  - `MainWindow._on_candidate_chosen(index, rang)`: `rec = self.playlist_panel.transition_recommendations[index]`; `k = next(d for d in rec.kandidaten if d["rang"] == rang)`; `candidate_choices.merke(rec.from_track.filePath, rec.to_track.filePath, t_out=k["t_out"], t_in=k["t_in"], blend_bars=k["blend_bars"])`; `mix_tips_panel.verwerfe_preview(index)`; `self._verteile_uebergaenge(self.current_bpm_tolerance, self.current_scoring_context)`; Status "Kandidat Rang n fuer Uebergang i→i+1 gewaehlt — Preview, Timeline und Export folgen." Verbindung `self.mix_tips_panel.candidate_chosen.connect(self._on_candidate_chosen)` im Aufbau.
  - `PlaylistPanel._populate_table`: Mix-In/Out-Spalten ueber `mixpunkte_fuer_tabelle(i, track, self.transition_recommendations)`; Tooltip Quelle; Bars-Anzeige ueber `seconds_to_bars(sekunden, track.bpm)` (vorhanden in `models`) — `mix_in_bars`/`mix_out_bars` nur als Fallback.
...
- [ ] **Step 1: Failing test** (reine `zusammenfassung(recs, tracks, dauer)`: zaehlt Paare, Paare mit Kandidat, Rang-1-Schemata, `bass_swap`-Anteil, Verletzungen = Plan-Punkte vor `intro_end` von B bzw. Blende ueber `outro_start` von A (Toleranz `QUANTIZE_TOLERANCE_SEC`, benannte Cues ausgenommen), Median Score).
...
Placeholder-Scan: keine TBD/TODO; Task 6/7/8 nennen die Helfer und Signaturen, der GUI-Feincode (Qt-Aufbau) folgt den in `main.py` vorhandenen Mustern (Karten :3481ff, Regler :1549ff) und wird an Tor 2 gegen diese Vorgaben geprueft.
