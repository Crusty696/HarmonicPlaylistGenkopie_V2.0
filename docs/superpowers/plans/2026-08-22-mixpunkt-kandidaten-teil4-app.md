...
18. **Gate `blende_ueber_b_ende`** (Praezisierung Spec Schritt 1, wie `crossfade_reserve` im Hoertest und der Playlist-Clamp): die Blende muss ab `in_b` vor dem Ende von Track B enden — sonst kuerzte `_clamp_transition_overlap` den Plan still, und die Kandidatentabelle zeigte eine andere Blende als der Renderer spielt.
19. **`_outro_overlap_limit` mit Gitter-Toleranz** (Diagnose der verbleibenden Plan-Overlap-Abweichungen: alle 12 kamen aus diesem Limit): die Funktion floorte den Kopfraum bis zum Outro ohne Toleranz auf ganze Takte; ein 1-ms-Rundungsrest (Teil 1 rundet `t` auf 3 Dezimalen) kostete einen ganzen Takt Blende. Jetzt `headroom + QUANTIZE_TOLERANCE_SEC` vor dem Floor — dieselbe Fehlerklasse wie der 3-ms-Phrasenfehler (`models.quantize_to_grid`). Bestandsverhalten aendert sich nur in diesem Randfall (Test `test_outro_overlap_limit_toleriert_rundungsrest`).
20. **Messwerkzeug vergleicht den aktiven Kandidaten** (`tools/playlist_kandidaten_messen.py`, Commit `86e61ad`): `zusammenfassung` las bisher `kandidaten[0]` (Rang 1) statt `kandidaten[kandidat_aktiv-1]` — seit der Kettenwahl (17) ist der aktive Kandidat nicht zwingend Rang 1, die gemeldeten 18 Overlap-Abweichungen waren dieser Messfehler. Korrigiert; neue Kennzahl `kette_neustarts` (Paare mit `kandidat_konsistent = False`). Endmessung 2026-08-22: 220/230 Paare mit Kandidat, Intro/Outro-Verletzungen 0, Overlap-Abweichungen 0, Cue-Gate 2 = Ketten-Neustarts 2, Score-Median 79 (ohne Kandidaten 83), Generierung 51 s (ohne 2 s).
