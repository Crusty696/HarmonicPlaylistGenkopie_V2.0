...
- Einheiten lokal: `energy_lokal` 0–100 (`calculate_energy`), `brightness_lokal` int (`calculate_brightness`), `avg_mids_lokal`/`avg_highs_lokal` aus `analyze_frequency_bands` (Band-Anteile in **Prozent 0–100**, `analysis.py:183-207`; lokal auf 3 Dezimalen gerundet), `bass_rms_dbfs` dBFS, `lufs_lokal` LUFS short-term.
